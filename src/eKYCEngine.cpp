...
eKYCEngine::eKYCEngine() : running_(false) {
    try {
        ...
        if (cfg.AERON_PROTOCOL == "ipc") {
            subscriptionChannel = "aeron:ipc";
            publicationChannel = "aeron:ipc";
        } else {
            subscriptionChannel = "aeron:udp?endpoint=" + cfg.SUBSCRIPTION_IP +
                                  ":" + std::to_string(cfg.SUBSCRIPTION_PORT);
            publicationChannel = "aeron:udp?endpoint=" + cfg.PUBLICATION_IP +
